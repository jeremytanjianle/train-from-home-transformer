...
def _sample_mask(seg, mask_alpha, mask_beta,
    ...
    """
    Blackcat uses n-gram masking SpanBERT(Joshi et al., 2019) w a 3-gram implementation
    """
    # initial masking array
    seg_len = len(seg)
    mask = np.array([False] * seg_len, dtype=np.bool)

    # keeps track of the number of tokens to be replaced by generator
    num_predict = 0

    # probability of being replaced is tracked in pvals
